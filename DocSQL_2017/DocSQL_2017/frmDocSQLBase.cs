...
{
	/// <summary>
	/// Summary description for frmiMaintStartBase.
	/// </summary>
	public class frmDocSQLBase : System.Windows.Forms.Form
	{
		...
		public frmDocSQLBase _callingForm = null;
...
//			this._guid = Guid.NewGuid();
			...
			this.Activated += new EventHandler(frmOBWClientBase_Activated);
...
//			this.KeyUp += new KeyEventHandler(frmLEADBase_KeyUp);
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmDocSQLBase));
			this.ucGrLbl = new mgGradientLabel();
			this.tips = new mgTooltip(this.components);
			this.SuspendLayout();
			// 
			// ucGrLbl
			// 
			...
			this.Name = "frmOBWClientBase";
			this.tips.SetSuperTip(this, null);
			this.Text = "frmOBWClientBase";
			this.Load += new System.EventHandler(this.frmOBWClientBase_Load);
			this.DoubleClick += new System.EventHandler(this.frmOBWClientBase_DoubleClick);
			this.Closing += new System.ComponentModel.CancelEventHandler(this.frmOBWClientBase_Closing);
			this.FormClosing += new System.Windows.Forms.FormClosingEventHandler(this.frmOBWClientBase_FormClosing);
			this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frmOBWClientBase_KeyDown);
		...
		private void frmOBWClientBase_Activated(object sender, EventArgs e)
		{
...
//		}
		...
		/// </summary>
		public frmDocSQLBase CallingForm
		{
...
//            catch (SqlException sqle) { Error.WriteErrorLog(sqle); }
		...
		/// <summary>
		/// Apply security based on the currently logged in user
		/// </summary>
		protected void ApplySecurity()
		{
			// Get out if we don't have a screen name
			//if (String.IsNullOrEmpty(this.SecurityScreenKeyName)) { return; }

			//// Enumerate through the collection and set all the controls
			//foreach (ScreenSecurityElement element in UserInterface.ScreenSecurityElements.GetByScreenKeyName(this.SecurityScreenKeyName))
			//{
			//	if (!String.IsNullOrEmpty(element.GridColumnName) &&
			//		!String.IsNullOrEmpty(element.ControlName))
			//	{
			//		// Try to find the grid control
			//		// Assume it's one of ours
			//		mgDevX_GridControl grid = (mgDevX_GridControl)FindControl(this.Controls, element.ControlName);
			//		if (grid != null)
			//		{
			//			if (element.AccessLevel < 5)
			//			{
			//				// They can't do anything with it - hide it
			//				grid.GridView.Columns[element.GridColumnName].VisibleIndex = -1;
			//				if (Utils.ListContainsString(grid.DisabledLinkColumns, element.GridColumnName)) { grid.DisabledLinkColumns.Remove(element.GridColumnName); }
			//			}
			//			else if (element.AccessLevel == 5)
			//			{
			//				// It's disabled - but they can see it
			//				if (!Utils.ListContainsString(grid.DisabledLinkColumns, element.GridColumnName)) { grid.DisabledLinkColumns.Add(element.GridColumnName); }
			//			}
			//			else if (element.AccessLevel == 9)
			//			{
			//				// They have full access - which should be the default state for the links
			//			}
			//		}
			//	}
			//}
		...
		/// <summary>
		/// Update the Main Status Bar in the application bar with text
		/// </summary>
		/// <param name="updateString">The update string to write</param>
		protected void UpdateMainStatusBar(string updateString)
		{
			//if (this.MdiParent != null &&
			//	typeof(IMDIExternalMethods).IsAssignableFrom(this.MdiParent.GetType()))
			//{
			//	((IMDIExternalMethods)this.MdiParent).SetStatusText(updateString);
			//}

			// Commented out by MG on 6/13/2008
			//((frmMDI)this.ParentForm).SetStatusText(updateString);
		...
		private void frmOBWClientBase_DoubleClick(object sender, System.EventArgs e)
		{
#if RELEASE
#else
		...
		private void frmOBWClientBase_Load(object sender, System.EventArgs e)
		{
		...
		private void frmOBWClientBase_Closing(object sender, System.ComponentModel.CancelEventArgs e)
		{
			if (this.DesignMode) { return; }
		...
		private void frmOBWClientBase_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			if (_closeOnEscape &&
		...
		private void frmOBWClientBase_FormClosing(object sender, FormClosingEventArgs e)
		{
			if (_askBeforeClosing && _isDirty)
			{
					...
					MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
				{
					...
					foreach (Control c in this.Controls)
					{
						...
						//if (c is IOBWClientDisposeObjects) { ((IOBWClientDisposeObjects)c).DisposeObjects(); }
					}
					e.Cancel = false;
				}
				else
		...
		private void CloseObjectsForControl(Control c)
		{
			if (c is DevExpress.XtraLayout.LayoutControl)
			{
				foreach (Control cEnum in c.Controls)
				{
					...
					//if (cEnum is IOBWClientDisposeObjects) { ((IOBWClientDisposeObjects)cEnum).DisposeObjects(); }
				}
			}
		}
	}
