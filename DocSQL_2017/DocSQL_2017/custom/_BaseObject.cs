...
{
	[Serializable]
    public class DocSQL_2017_BaseObject : IDisposable, 
		IClassGenCloneable, IClassGenPropertyChanged, INotifyPropertyChanged, IDXDataErrorInfo
    {
		...
		public bool IsValid
		{
			get 
			{ 
				if (this._disposed) { throw new ObjectDisposedException("DocSQL_2017_BaseObject", _objectDisposedMessage); }
        ...
        public RecordStatus RecordStatus
        {
			...
			{ 
				...
				if (value != RecordStatus.Current)
				{
					...
					if (!RecordStatusParentHolders.TryGetValue(this.GetType(), out props))
					{
						...
						foreach (PropertyInfo prop in this.GetType().GetProperties())
						{
							if (prop.Name.EndsWith("Item") &&
								prop.PropertyType.FullName.Contains("DocSQL_2017.") &&
					...
					{
						if ((DocSQL_2017_BaseObject)prop.GetValue(this, null) != null)
						{
							switch (((DocSQL_2017_BaseObject)prop.GetValue(this, null)).RecordStatus)
							{
								case RecordStatus.Current:
									((DocSQL_2017_BaseObject)prop.GetValue(this, null)).RecordStatus = RecordStatus.Modified;
									break;
								default:
									((DocSQL_2017_BaseObject)prop.GetValue(this, null)).RecordStatus =
										((DocSQL_2017_BaseObject)prop.GetValue(this, null)).RecordStatus;
									break;
							}
						}
		...
		//{
		//    if (!DocSQL_2017_BaseObject.PropInfoArray.ContainsKey(typeName))
		//    {
		//        // Load them up
		//        Type t = System.Type.GetType("DocSQL_2017." + typeName);
		...
		//            }
		//        //}
		//        //((DocSQL_2017_BaseObject)o).Dispose();		// Dispose of the object
		//        DocSQL_2017_BaseObject.PropInfoArray.Add(typeName, newProps);		// Add the new collection
		//    }
		//}
...
#region ClassGenBindingList 
	...
	public interface IClassGenBindingList<T, K> where T : DocSQL_2017_BaseObject
	{
		// Methods
	...
	public class ClassGenBindingList<T, K> : BindingList<T>, IClassGenBindingList<T, K>, IClassGenBindingList,
		...
		where T : DocSQL_2017_BaseObject
	{
		...
		public new void Insert(int index, T item)
		{
			...
			{
				// Log the error
				throw new DocSQL_2017Exception("The key: " + ((K)item.GetType().GetProperty(KeyFieldName).GetValue(item, null)).ToString() +
		...
		public bool IsCurrent
		{
			get 
			{
				if (!(typeof(DocSQL_2017_BaseObject).IsAssignableFrom(typeof(T))))
				{
					return true;
				}
				foreach (T item in this)
				{
					if ((item as DocSQL_2017_BaseObject).RecordStatus != RecordStatus.Current)
					{
						return false;
					}
				}
		...
		public List<string> PropertyValueExistsMoreThanOnceInCollection(string propertyName, bool sortDescending)
		{
			...
			foreach (T item in this)
			{
				if ((item as DocSQL_2017_BaseObject).RecordStatus != RecordStatus.Deleted)
				{
