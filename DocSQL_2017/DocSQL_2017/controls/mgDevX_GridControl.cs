...
namespace DocSQL_2017
{
	public class mgDevX_GridControl : DevExpress.XtraGrid.GridControl, IXtraResizableControl
	{
			...
			: base()
		{
			...
			_imageLibrary.Add(GridLinkType.Add, global::DocSQL_2017.res16x16.add2);
			_imageLibrary.Add(GridLinkType.Delete, global::DocSQL_2017.res16x16.delete2);
			_imageLibrary.Add(GridLinkType.Edit, global::DocSQL_2017.res16x16.pencil);
			_imageLibrary.Add(GridLinkType.Print, global::DocSQL_2017.res16x16.printer);
		...
		{
			bool rtv = true;
			if (DocSQL_2017.Utils.ListContainsString(_disabledLinkColumns, columnName)) { rtv = false; }
			return rtv;
		}
		...
		{
			if (DocSQL_2017.Utils.ListContainsString(_disabledLinkColumns, columnName)) 
			{
		...
		private void view_ShowingEditor(object sender, CancelEventArgs e)
		{
				...
				this.DataSource is IBindingList)
			{
				...
				if (list.Count > 0 &&
					list[0] is DocSQL_2017_BaseObject)
				{
					// Get the row we're trying to handle here
					DocSQL_2017_BaseObject obj = (DocSQL_2017_BaseObject)view.GetRow(view.FocusedRowHandle);
					if (obj != null &&
		...
		private void view_CustomDrawColumnHeader(object sender, ColumnHeaderCustomDrawEventArgs e)
		{
				...
				!e.Info.CustomizationForm)
			{
				...
				Image img = global::DocSQL_2017.res16x16.Check_Header;
        ...
        private void ClearChecked()
        {
				...
				this.DataSource is IBindingList)
			{
				...
				if (list.Count > 0 &&
					list[0] is DocSQL_2017_BaseObject)
				{
					foreach (DocSQL_2017_BaseObject obj in list)
					{
		...
		public void CustomDrawCell(object sender, RowCellCustomDrawEventArgs e)
		{
			...
			if (link != null)
			{
				...
				if (link.LinkImage != null)
				{
					...
					if (DocSQL_2017.Utils.ListContainsString(_disabledLinkColumns, e.Column.FieldName)) { img = UserInterface.GetGrayscaleImage(img); }
				...
				if (totalWidth < r.Width)
				{
					...
					Color textColor = DocSQL_2017.Utils.GetTextColorBasedOnBackgroundColor(e.Appearance.BackColor, Color.White, Color.Blue);
					...
					if (DocSQL_2017.Utils.ListContainsString(_disabledLinkColumns, e.Column.FieldName)) 
					{
					...
					columnName.ToLower().StartsWith("gridcustom_"))
				{
						...
						this.DataSource is IBindingList)
					{
							...
							list[0] is DocSQL_2017_BaseObject)
						{
							...
							{
								if (columnName == "GridCustom_0" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_0)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_0; break; }
								if (columnName == "GridCustom_1" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_1)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_1; break; }
								if (columnName == "GridCustom_2" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_2)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_2; break; }
								if (columnName == "GridCustom_3" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_3)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_3; break; }
								if (columnName == "GridCustom_4" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_4)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_4; break; }

								if (columnName == "GridCustom_5" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_5)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_5; break; }
								if (columnName == "GridCustom_6" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_6)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_6; break; }
								if (columnName == "GridCustom_7" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_7)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_7; break; }
								if (columnName == "GridCustom_8" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_8)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_8; break; }
								if (columnName == "GridCustom_9" && !String.IsNullOrEmpty(((DocSQL_2017_BaseObject)list[i]).GridCustom_9)) { text = ((DocSQL_2017_BaseObject)list[i]).GridCustom_9; break; }
							}
						}
					}
			...
			DevExpress.XtraEditors.ImageLocation location)
		{
			if (view is LayoutView)
			{
				try
				{
					...
					////RepositoryItemDocSQL_ButtonEdit riButtonEdit = this.RepositoryItems.Add("DocSQL_ButtonEdit") as RepositoryItemDocSQL_ButtonEdit;
					//RepositoryItemDocSQL_ButtonEdit riButtonEdit = new RepositoryItemDocSQL_ButtonEdit();
				...
				else
				{
					...
					switch (format)
					{
						...
						case GridEditorFormat.DateTime:
							...
							datetimeEdit.Mask.EditMask = DocSQL_2017.Utils.GetDateAndTimeFormatAsString();
						...
						case GridEditorFormat.Date:
							...
							dateEdit.Mask.EditMask = DocSQL_2017.Utils.GetDateFormatAsString();
						...
						case GridEditorFormat.Time:
							...
							timeEdit.Mask.EditMask = DocSQL_2017.Utils.GetTimeFormatAsString();
				...
				else
				{
					...
					switch (format)
					{
						...
						case GridDisplayFormat.DateTime:
							((GridView)view).Columns[columnFieldName].DisplayFormat.FormatType = FormatType.Custom;
							((GridView)view).Columns[columnFieldName].DisplayFormat.FormatString = DocSQL_2017.Utils.GetDateAndTimeFormatAsString();
							break;

						case GridDisplayFormat.Date:
							((GridView)view).Columns[columnFieldName].DisplayFormat.FormatType = FormatType.Custom;
							((GridView)view).Columns[columnFieldName].DisplayFormat.FormatString = DocSQL_2017.Utils.GetDateFormatAsString();
							break;

						case GridDisplayFormat.Time:
							((GridView)view).Columns[columnFieldName].DisplayFormat.FormatType = FormatType.Custom;
							((GridView)view).Columns[columnFieldName].DisplayFormat.FormatString = DocSQL_2017.Utils.GetTimeFormatAsString();
							break;
		...
		public void SetCaptionsAuto(BaseView view)
		{
			if (view is GridView)
			{
				foreach (GridColumn col in ((GridView)view).Columns)
				{
					...
					{
						// Set the caption
						col.Caption = DocSQL_2017.Utils.AlterCaptionFromFieldName(col.FieldName);
					}
				}
			}
		}
	...
	//public class DocSQL_ButtonEdit : ButtonEdit
	//{
	//    static DocSQL_ButtonEdit()
	//    {
	//        RepositoryItemDocSQL_ButtonEdit.Register();
	//    }


	//    public override string EditorTypeName
	//    {
	//        get { return RepositoryItemDocSQL_ButtonEdit.EditorName; }
	//    }
	//    [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
	//    public new RepositoryItemDocSQL_ButtonEdit Properties
	//    {
	//        get { return base.Properties as RepositoryItemDocSQL_ButtonEdit; }
	//    }
	//}

	//public class DocSQL_ButtonEditPainter : ButtonEditPainter
	//{

	//    public DocSQL_ButtonEditPainter()
	//    {
	...
	//public class DocSQL_ButtonEditViewInfo : ButtonEditViewInfo
	//{
	//    public RepositoryItemDocSQL_ButtonEdit DocSQL_RepositoryItem
	//    {
	//        get { return this.Item as RepositoryItemDocSQL_ButtonEdit; }
	//    }

	//    public DocSQL_ButtonEditViewInfo(RepositoryItem item)
	//        : base(item)
	//    {
	...
	//        int buttonsHeight = DocSQL_RepositoryItem.ButtonsHeight;
	//        if (buttonsHeight > 0 && buttonsHeight < buttonBounds.Height)
	//            r.Height = buttonsHeight;
	//        if (buttonsHeight > 0)
	//            r.Height = buttonsHeight;

	//        if (DocSQL_RepositoryItem.ButtonsAlignment == DevExpress.Utils.VertAlignment.Bottom)
	//            r.Y = buttonBounds.Bottom - buttonsHeight;
	//        if (DocSQL_RepositoryItem.ButtonsAlignment == DevExpress.Utils.VertAlignment.Center)
	...
	//public class RepositoryItemDocSQL_ButtonEdit : RepositoryItemButtonEdit
	//{
	//    static RepositoryItemDocSQL_ButtonEdit()
	//    {
	//        Register();
	//    }
	//    public RepositoryItemDocSQL_ButtonEdit() { }

	//    internal const string EditorName = "DocSQL_ButtonEdit";

	//    public static void Register()
	//    {
	//        EditorRegistrationInfo.Default.Editors.Add(new EditorClassInfo(EditorName, typeof(DocSQL_ButtonEdit),
	//            typeof(RepositoryItemDocSQL_ButtonEdit), typeof(DocSQL_ButtonEditViewInfo),
	//            new DocSQL_ButtonEditPainter(), true, null));
	//    }
	//    public override string EditorTypeName
	//    {
